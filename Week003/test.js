...
var one = $.foldFromRight($.sum, 0, [1, 2, 3, 4, 5])
assertEq(one, 15, "foldr sum");

var two = $.foldFromLeft($.iif($.comp(isUpperCase, $.arg1), $.id, $.concat), [], "Hello, Moto".split(""));
assertEq(two, "ellooto".split(""), "foldl only lowercase alpha chars");

var three = $.mapFromLeft($.mul(10), [1, 2, 3, 4])
assertEq(three, [10, 20, 30, 40], "map from left *10");

var fourA = $.filterFromLeft($.comp($.eq(1), $.flip($.mod)(2)), [1, 2, 3, 4])
assertEq(fourA, [1,3], "filter out even values");

var fourB = $.filterFromLeft($.lt(10), [1, 2, 3, 4]);
assertEq(fourB, [], "filters everything out");

var fiveA = $.findFromLeft($.eq(2), [1, 2, 3, 4]);
assertEq(fiveA, 2, "match w/find");

var fiveB = $.findFromLeft($.eq(10), [1, 2, 3, 4]);
